...
def get_pet_labels(image_dir):
    ...
    for item in range(0, len(in_files), 1):
       
       # Skips file if starts with . (like .DS_Store of Mac OSX) because it 
       # isn't an pet image file
       if in_files[item][0] != ".":
           temps=in_files[item].lower().split("_")
       ...
       if in_files[item] not in results_dic:
              results_dic[in_files[item]] = [pet_label]
              
       else:
               print("** Warning: Duplicate files exist in directory:",in_files[item])
